...
// Multiple accepts a slice of strings and stems each of them.
func Multiple(words []string) (output []string) {
...
// MultipleMutate accepts a pointer to a slice of strings and stems them in place.
// It modifies the original slice.
func MultipleMutate(words *[]string) {
	for idx, word := range *words {
		(*words)[idx] = Do(word)
	}
}

// Concurrent accepts a pointer to a slice of strings and stems them in place.
